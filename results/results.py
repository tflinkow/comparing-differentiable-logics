...
def format_value(v) -> str:
    return 'nan' if v == -1 else f'\qty{{{v * 100:.2f}}}{{\percent}}'
...
"""
        ...
        def write(report, key: str):
            ...
            p_acc = df['Test-P-Acc'].values[-(len(df) // 10):]
            ...
            if 'Baseline' in report:
                file.write(fr'\addplot+[mark indices={best_epoch}, densely dotted] table [y={key}] {{{full_path(report)}}};' + '\n')
            else:
                file.write(fr'\addplot+[mark indices={best_epoch}] table [y={key}] {{{full_path(report)}}};' + '\n')

        for report in report_files:
            print(f'reading {os.path.join(report_dir, report)}')
            write(report, key='Test-P-Acc')
...
def write_table_file(report_dir: str, target_file: str):
    ...
    for report in report_files:
        ...
        results[full_path(report)] = Result(p_acc[i], c_acc[i], c_sec[i], p_acc[i] * c_acc[i] * c_sec[i])
        ...
        print(f'file: {report} avg. train time [s]: {avg_train_time:.2f} avg. test time [s]: {avg_test_time:.2f} total train time [s]: {total_train_time:.2f} total test time [s]: {total_test_time:.2f}')
...
"""
            ...
            for key, value in results.items():
                if key == best:
                    file.write(fr'{get_name_from_file(key)} & \textbf{{{format_value(value.p_acc)}}} & \textbf{{{format_value(value.c_acc)}}} & \textbf{{{format_value(value.c_sec)}}} \\' + '\n')
                else:
                    file.write(fr'{get_name_from_file(key)} & {format_value(value.p_acc)} & {format_value(value.c_acc)} & {format_value(value.c_sec)} \\' + '\n')
